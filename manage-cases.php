<?php
    ...
    $crudObj = new CRUD('cases_view', 'case_id');
    $crudObj->mysqli = $link;

    $records = $crudObj->FindAll('cases_view', array(), array('organization_id='.$org_id), 0, 0, array(array('case_number', 'DESC')));

        //select query statement
        //$sql = "SELECT case_view.*, cases.case_subject 
        //FROM case_view
        //INNER JOIN cases ON case_view.case_id = cases.id";
    ...
    ?><head>
        <title>Case Manager - All Cases</title>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                    <h4 class="mb-sm-0 font-size-18">All Cases</h4>
                                ...
                                <table id="datatable-buttons" class="table table-sm table-striped table-bordered dt-responsive nowrap w-100 mt-3 mb-3">
                                    ...
                                    if(isset($records) && count($records)>0){
                                        foreach($records as $record){
                                            $nameArr = array();
                                            if(isset($record['fname']) && !empty($record['fname']))
                                                $nameArr[] = $record['fname'];
                                            if(isset($record['mname']) && !empty($record['mname']))
                                                $nameArr[] = $record['mname'];
                                            if(isset($record['lname']) && !empty($record['lname']))
                                                $nameArr[] = $record['lname'];

                                            $client_name = $record["client_firstname"] . ' ' . $record["client_lastname"];
                                            ?><tr>
                                                <td><a class="nav-link active" href="cases-view.php?case_id=<?php echo $record['case_id']; ?>">
                                                ...
                                                <td><?php echo $record['case_subject']; ?></td>
                                                <td><?php echo $record['case_status_name']; ?></td>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-xxl-4 col-lg-4 col-md-4">
                                <label for="case_type_id" class="form-label">Case Subject</label>
                                <input type="text" readonly disabled class="form-control" id="case_subject" value="">
                            </div>
                ...
                $.ajax({
                    ...
                    success : function(data) {
                        if(data['status']==true){
                            ...
                            $('#case_subject').val(data['data']['case_subject']);
